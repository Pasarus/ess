...
def filter_by_time(da: sc.DataArray,
    ...
    Args:
        ...
        start: Any convertible to a datetime64 Scalar variable e.g. str or sc.Variable,
         input using absolute time format similar to Mantid, e.g. 2011-08-13T17:11:30 or
         any other format that can be converted into a numpy datetime64 variable. The
         start of the time_dim_name dimension, in the output

        end: Any, input format the same as start. The end of the time_dim_name dimension
         in the output

        time_dim_name: str, The time dimension name, defaults to "pulse_time"

        datetime_unit: str, The time unit used in the time_dim_name dimension, e.g.
         nanoseconds = "ns" or seconds = "s", defaults to "ns". Only needed if you pass
         not already constructed start or end i.e. a string instead of a Scalar variable
         containing datetime64.

    Returns:
        The output of the sc.bin after filtering out everything between the start and
         end args from the input da
    """
...
def filter_attribute_by_time(da: sc.DataArray,
    ...
    Args:
        ...
        attribute_name: str, The name of the attribute to be filtered from the DataArray
         da.
        ...
        time_name: str, The name of the time coordinate in the attribute, defaults to
         "time"
    ...
    Returns:
        sc.DataArray or sc.Variable, whichever the attribute is in the original da that
         is passed.
    """
    start, end = _input_processing_time_conversion(start, end, datetime_unit)
...
def filter_attribute_by_value(da: sc.DataArray, attribute_name: str,
                              min_value: sc.Variable, max_value: sc.Variable):
    """
    Return all data points between a minimum and maximum values of an attribute in a
     DataArray
...
def filter_data_array_on_attribute(da: sc.DataArray,
    ...
    Args:
        ...
        good_data_lut: Any, a definition of what is good data, using the boolean
        ...
        mapping_coord: str, The coordinate that connects the attribute, and the data
         together, this is often the, pulse_time, you may need to rename the attribute's
         coordinate to support this.

        data_slices_name: str, The name of the dimension that will be added to the
         passed sc.DataArray, da, that can then be sliced later for good and bad data.

    Returns:
        The originally passed sc.DataArray, da, with an extra dimension that can then be
         sliced for good data on slice 1, and sliced for bad data on slice 0. For
    ...
    good_data = sc.concatenate(good_data_lut[mapping_coord, 0], good_data,
                               mapping_coord)
    good_data = sc.concatenate(good_data, good_data_lut[mapping_coord, -1],
                               mapping_coord)
